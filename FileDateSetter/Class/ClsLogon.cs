...
// 2026/08/09 Gemini 3.6 Flash (High) Review & Modified
// 2026/08/15 AllowUnsafeBlocksのfalse対応
...
namespace FileDateSetter.Class
{
    ...
    public class ClsLogon : IDisposable
    {
        ...
        /// </summary>
        /// <param name="mainProc">成り代わりユーザー権限で実行するメイン処理オブジェクト</param>
        /// <example>
        /// <code>
        /// ClsLogon logon = new ClsLogon();
        ...
        public void Execute(ClsFind mainProc)
        {
            ...
            WindowsIdentity.RunImpersonated(
                ...
                () =>
                {
                    if (_verbose > _debugThreshold)
                    {
                        Console.Out.WriteLine("[ClsLogon.Execute()] START : WindowsIdentity.RunImpersonated() -> ClsFind.Execute()");
                    }
                    _returnCode = mainProc.Execute();

                    if (_verbose > _debugThreshold)
                    {
                        Console.Out.WriteLine($"[ClsLogon.Execute()] E N D : WindowsIdentity.RunImpersonated() -> ClsFind.Execute() -> RETURN CODE = {_returnCode}");
                    }
                }
            );
        }
